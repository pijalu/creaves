...
func (v DrugsResource) saveDrugs(d *models.Drug, c buffalo.Context) (*validate.Errors, error) {
	...
	if !ok {
		return &validate.Errors{}, fmt.Errorf("no transaction found")
	}

	if d.ID == uuid.Nil {
		if verrs, err := tx.ValidateAndCreate(d); err != nil || verrs.HasAny() {
			return verrs, err
		}
	} else {
		if verrs, err := tx.ValidateAndUpdate(d); err != nil || verrs.HasAny() {
			return verrs, err
		}
	}
	...
	for i := 0; i < len(d.Dosages); i++ {
		...
		if ds.ID == uuid.Nil {
			if verrs, err := tx.ValidateAndCreate(ds); err != nil || verrs.HasAny() {
				return verrs, err
			}
		} else {
			if verrs, err := tx.ValidateAndUpdate(ds); err != nil || verrs.HasAny() {
				return verrs, err
			}
		}
	}
...
func (v DrugsResource) Destroy(c buffalo.Context) error {
	...
	if err := tx.Destroy(drug.Dosages); err != nil {
		return err
	}
